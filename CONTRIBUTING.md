...
Given a sustainability general issue for Python, saying "Busy-Wait Loop", put the query `busy-wait-loop.ql` and its sibling files (.py, .expected) for unit testing in their own directory.
```
...
Your pull request will be reviewed, and if it is accepted and merged, the rest of the process is automated. Our workflows check that the unit tests pass (please run tests locally before), and if they do, your new query is added to the `main` branch, and a new version of the relevant Green CodeQL pack is published.
